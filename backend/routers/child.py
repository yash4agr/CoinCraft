...
class ChildDashboardData:
    def __init__(self, user: User, profile: ChildProfile, goals: List[Goal], 
                 transactions: List[Transaction], achievements: List[UserAchievement]):
        self.user = user
...
async def get_child_dashboard(
    ...
    # Get completed goals
    completed_goals_stmt = select(Goal).where(
        and_(Goal.user_id == current_user.id, Goal.is_completed == True)
    ).order_by(Goal.updated_at.desc()).limit(5)
    ...
    learning_journey = [
        {
            ...
            "status": "completed" if any(p.is_completed for p in user_progress.values()) else "available",
...
async def get_child_stats(
    ...
    if current_user.role not in ['younger_child', 'older_child']:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only children can view stats"
        )
    ...
    transactions_stmt = select(Transaction).where(Transaction.user_id == current_user.id)
    transactions_result = await session.execute(transactions_stmt)
    transactions = transactions_result.scalars().all()
    ...
    return {
        ...
        "activities": {
            ...
            "total_available": 10  # Hardcoded for now
        },
        "transactions": {
