...
async def get_teen_dashboard(
    ...
    if not child_profile:
        child_profile = ChildProfile(
            user_id=current_user.id, age=15, coins=0, level=1, streak_days=0
        )
    ...
    categorized_goals = {"saving": [], "spending": [], "wants": []}
    ...
    return {
        ...
        "budget_overview": {
            ...
            "percentages": {"saving": 40, "spending": 35, "wants": 25},
        },
        ...
        "stats": {
            ...
            "completed_activities": len(
                [p for p in user_progress.values() if p.is_completed]
            ),
            "streak_days": child_profile.streak_days,
            "level": child_profile.level,
        },
...
async def get_teen_budget(
    ...
    # Get user's budget categories or create defaults
    budget_stmt = select(BudgetCategory).where(
        BudgetCategory.user_id == current_user.id
    )
    budget_result = await session.execute(budget_stmt)
    budget_categories = budget_result.scalars().all()
    ...
    if not budget_categories:
        default_categories = [
            BudgetCategory(
                ...
                name="saving",
                budget_amount=int(total_coins * 0.4),
            ...
            BudgetCategory(
                user_id=current_user.id,
                name="spending",
                budget_amount=int(total_coins * 0.35),
            ...
            BudgetCategory(
                user_id=current_user.id,
                name="wants",
                budget_amount=int(total_coins * 0.25),
                icon="ri-heart-line",
                color="#7c3aed",
            ),
        ]
    ...
    # Calculate budget allocations from database
    total_budget = sum(cat.budget_amount for cat in budget_categories)
    budget_allocations = {}
    percentages = {}

    for category in budget_categories:
        budget_allocations[category.name] = category.budget_amount
        percentages[category.name] = (
            int((category.budget_amount / total_budget * 100))
            if total_budget > 0
            else 0
        )

    categorized_transactions = {"saving": [], "spending": [], "wants": []}
...
async def update_teen_budget(
    ...
    print(
        f"[BACKEND] Updated teen budget: {saving}% saving, {spending}% spending, {wants}% wants"
    )

    return {
        "message": "Budget updated successfully",
        "budget_allocations": new_allocations,
        "percentages": {"saving": saving, "spending": spending, "wants": wants},
    }
...
async def create_teen_goal(
    ...
    if goal_data.get("deadline"):
        new_goal.deadline = datetime.fromisoformat(
            goal_data["deadline"].replace("Z", "+00:00")
        )
...
async def get_teen_activities(
    ...
    if duration:
        if duration == "short":
            ...
            query = query.where(
                and_(Module.estimated_duration >= 15, Module.estimated_duration <= 30)
            )
        elif duration == "long":
    ...
    if search:
        ...
        query = query.where(
            or_(Module.title.ilike(search_term), Module.description.ilike(search_term))
        )
...
async def create_conversion_request(
    ...
    if coin_amount <= 0 or dollar_amount <= 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid amounts"
        )
    ...
    print(
        f"[BACKEND] Created conversion request: {coin_amount} coins for ${dollar_amount}"
    )
        ...
        requests_data.append(
            {
                ...
                "responseDate": request.updated_at.isoformat()
...
async def get_teen_analytics(
    ...
    transactions_stmt = select(Transaction).where(
        and_(
            Transaction.user_id == current_user.id, Transaction.created_at >= start_date
        )
    )
    transactions_result = await session.execute(transactions_stmt)
    transactions = transactions_result.scalars().all()
    ...
    return {
        ...
        "spending_patterns": {
            "by_category": {"saving": total_saved, "spending": total_spent, "wants": 0}
        },
    }
